...
app.post('/api/submitOrder', (req, res) => {
  ...
  db.query(ordersQuery, [values], (err, result) => {
    ...
    res.send({ message: 'Order submitted successfully', trackingNumber });
  });
});
...
app.post('/api/upload-payment', upload.single('payment_receipt'), (req, res) => {
  ...
  const updateQuery = 'UPDATE order_details SET receipt_path = ?, status = "underprocess" WHERE invoice_no = ?';

  db.query(updateQuery, [filePath, order_tracking_number], (error, results) => {
    if (error) {
      console.error('Error updating receipt path:', error);
      return res.status(500).json({ message: 'Error updating receipt path. Please try again.' });
    }
...
app.get('/api/confirmedOrders', (req, res) => {
  ...
  const query = 'SELECT * FROM order_details WHERE user_id = ? and status = "pending"';
  db.query(query, [userId], (err, results) => {
    if (err) throw err;
    res.json(results);
  });
...
app.put('/api/orders/:order_id/generateInvoice', (req, res) => {
  ...
  // Fetch order details and calculate total amount
  const fetchOrderQuery = 'SELECT * FROM orders WHERE order_id = ?';
  db.query(fetchOrderQuery, [order_id], (err, orders) => {
    if (err) {
    ...
    const totalAmount = orders.reduce((sum, order) => sum + order.PRICE_AED * order.QUANTITY, 0);
    const trackingNumber = orders[0].order_tracking_number;
    const userId = orders[0].id;

    // Check if invoice_no already exists in order_details
    const checkInvoiceQuery = 'SELECT * FROM order_details WHERE invoice_no = ? and status="pending"';
    db.query(checkInvoiceQuery, [trackingNumber], (err, orderDetails) => {
      if (err) {
      ...
      if (orderDetails.length > 0) {
        // If invoice_no exists, update total_amount
        const existingTotal = orderDetails[0].total_amount;
        const newTotalAmount = existingTotal + totalAmount;

        const updateOrderDetailsQuery = 'UPDATE order_details SET total_amount = ? WHERE invoice_no = ? and status = "pending"';
        db.query(updateOrderDetailsQuery, [newTotalAmount, trackingNumber], (err, result) => {
          if (err) {
            console.error('Error updating order details:', err);
            return res.status(500).send({ message: 'Error updating order details' });
          }

          // Update order status after successfully updating order details
          updateOrderStatus(order_id, res, newTotalAmount);
        });
      } else {
        // If invoice_no does not exist, insert into order_details
        const insertOrderDetailsQuery = 'INSERT INTO order_details (invoice_no, total_amount, user_id,status) VALUES (?, ?, ?,"pending")';
        db.query(insertOrderDetailsQuery, [trackingNumber, totalAmount, userId], (err, result) => {
          if (err) {
            console.error('Error inserting order details:', err);
            return res.status(500).send({ message: 'Error generating invoice' });
          }

          // Update order status after successfully inserting order details
          updateOrderStatus(order_id, res, totalAmount);
        });
      }
    });
  });
...
const updateOrderStatus = (order_id, res, totalAmount) => {
  const updateOrderQuery = 'UPDATE orders SET status = "confirmed" WHERE order_id = ?';
  db.query(updateOrderQuery, [order_id], (err, results) => {
    if (err) {
    ...
    res.json({ message: 'Order status updated to confirmed and invoice processed', totalAmount });
  });
};
